...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['mensaje'])) {
    ...
    function stockProducto($conn, $mensaje) {
        ...
        if ($id_producto) {
            $stmt = $conn->prepare("SELECT nombre_producto, cantidad, foto FROM productos WHERE id_producto = ? LIMIT 1");
            ...
            $sql = "SELECT nombre_producto, cantidad, foto FROM productos WHERE nombre_producto LIKE '%$producto_esc%' LIMIT 1";
            $res = $conn->query($sql);
            if ($res && $res->num_rows > 0) {
                $row = $res->fetch_assoc();
                $img = $row['foto'] ? $row['foto'] : '';
    ...
    <style>
        ...
        * {
        ...
        #chatPanel form {
            display: flex;
            padding: 15px 20px;
            border-top: 1px solid var(--border-color, #e6e5e5);
                ...
                <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-6 animate-slide-up border border-gray-100 dark:border-gray-700">
                    ...
                    <div class="h-64 flex items-center justify-center">
                        <div class="text-center">
                            <div class="w-20 h-20 bg-gradient-primary rounded-full flex items-center justify-center mx-auto mb-4 animate-bounce-light">
                                <i class="fas fa-chart-line text-white text-2xl"></i>
                            </div>
                            <p class="text-gray-500 dark:text-gray-400">Gráfico de ventas próximamente</p>
                        </div>
                    </div>
                </div>
