...
if (isset($_GET['reporte'])) {
    ...
    class PDF extends FPDF {
        function Header() {
            ...
            $this->Cell(0, 10, 'REPORTE DE CATEGORIAS - PIL ANDINA', 0, 1, 'C');
            $this->Ln(5);
        }
    ...
    $pdf->Cell(150, 10, 'NOMBRE CATEGORIA', 1, 1, 'C', true);
    $pdf->SetFont('Arial', '', 10);
    $pdf->SetTextColor(0, 0, 0);
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        $pdf->Cell(30, 8, ++$row_num, 1, 0, 'C', $fill);
        $pdf->Cell(150, 8, $row['nombre_categoria'], 1, 1, 'L', $fill);
    }

    $pdf->Output('I', 'Reporte_Categorias_' . $codigo_reporte . '.pdf');
    if (isset($stmt)) {
        $stmt->close();
    }
    exit();
